...
class A3TGCN(torch.nn.Module):
    ...
    def _setup_layers(self):
        ...
        self.attention = torch.nn.Parameter(torch.empty(self.periods))
...
class EEGModel(nn.Module):
    def __init__(self, num_nodes, node_features, num_classes, num_windows, device):
        ...
        self.forwardA3TGCN = A3TGCN(in_channels=node_features, out_channels=32, periods=num_windows, num_nodes=num_nodes) # node_features=51, periods=50
        self.backwardA3TGCN = A3TGCN(in_channels=node_features, out_channels=32, periods=num_windows, num_nodes=num_nodes) # node_features=51, periods=50
        self.num_nodes = num_nodes
    ...
    def forward(self, X, A):
        ...
        HS1 = torch.nn.functional.dropout(HS1, p=0.5)
        X_flip = torch.flip(X, dims=[1])
        A_flip = torch.flip(A, dims=[1])
        HS2 = self.backwardA3TGCN(X_flip, A_flip)
        HS2 = torch.nn.functional.dropout(HS2, p=0.5)
