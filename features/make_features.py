...
def detrend_3d(X):
    """
    Remove linear trend (DC drift) from a 3D array of time-series data.

    Parameters:
        X (np.ndarray): Array of shape (n_samples, n_channels, n_timesteps)
    ...
    # Apply scipy.signal.detrend along the time axis (axis=2)
    for i in range(n_samples):
        X_detrended[i] = signal.detrend(X[i], axis=1)
